...
$stmt = $conn->query("SELECT * FROM course ORDER BY course_name");
...
if (isset($_POST['record_course'])) {
    ...
    foreach ($data['course'] as $key => $value) {
        $stmt = $conn->query("SELECT course_name FROM course WHERE course_name = '$value' ");
    ...
    if (empty($_SESSION["record_course_msg"])) {
        ...
        foreach ($data['course'] as $key => $value) {
            $stmt = $conn->query("INSERT INTO course (course_name) VALUES ('$value') ");
