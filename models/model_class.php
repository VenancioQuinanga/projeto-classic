...
$stmt = $conn->query("SELECT * FROM class ORDER BY class_name");
$class = $stmt->fetchAll();
...
if (isset($_POST['record_class'])) {
    ...
    foreach ($data['class'] as $key => $value) {
        $stmt = $conn->query("SELECT class_name FROM class WHERE class_name = '$value' ");
    ...
    if (empty($_SESSION["record_class_msg"])) {
        ...
        foreach ($data['class'] as $key => $value) {
            $stmt = $conn->query("INSERT INTO class (class_name) VALUES ('$value') ");
